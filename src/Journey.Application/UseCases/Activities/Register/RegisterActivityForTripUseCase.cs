...
namespace Journey.Application.UseCases.Activities.Register
{
    public class RegisterActivityForTripUseCase
    {
        ...
        private void Validate(Trip? trip, RequestRegisterActivityJson request)
        {
            ...
            if ((request.Date.Date >= trip.StartDate.Date && request.Date.Date <= trip.EndDate.Date) == false)
            {
            ...
            {
                var errorMesages = result.Errors.Select(error => error.ErrorMessage).ToList();
